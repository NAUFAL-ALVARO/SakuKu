<x-app-layout>
    ...
    <div class="py-6 max-w-md mx-auto sm:px-6">
        <form action="{{ route('categories.update', $category) }}" method="POST" class="bg-white shadow p-6 rounded" x-data="{ loading: false }" @submit="loading = true">
            @csrf @method('PUT')
             @if ($errors->any())
            ...
            <select name="tipe" class="w-full border rounded p-2 mb-4" required>
                <option value="income" {{ $category->tipe === 'income' ? 'selected' : '' }}>Pemasukan</option>
                <option value="expense" {{ $category->tipe === 'expense' ? 'selected' : '' }}>Pengeluaran</option>
            </select>
